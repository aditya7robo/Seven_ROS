...
/** The following implementation of delegates was copy pasted from 
 	* https://codereview.stackexchange.com/questions/14730/impossibly-fast-delegate-in-c11
	* Works in the same way as Don clugstons delegate implementation.
	* i.e. static_cast a void pointer to the object whose member function
	* we would like to bind to and invoke. This trick is done within a 
	* static member function of the delegate class. So instead of using macros 
	* this version of delegates uses template specialization and type traits to 
	* bind various types of function signatures eg: const member fn's, non-const 
	* member fn's, free functions, lambdas and closures. For lambdas and closures 
	* which are passed as rvalues a special trick is used. 
	* memory is allocated for the size of the lambda and then the lambda is initialized
	* using the placement new operator. The type of the deleter is passed to the shared_ptr
	* which manages the lambda's memory.
	*/
